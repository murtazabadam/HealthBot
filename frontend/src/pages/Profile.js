...
import {
  Activity, User, Mail, Phone, MapPin, Droplet,
  Calendar, Lock, Eye, EyeOff, CheckCircle2,
  ArrowLeft, Edit3, Save, X, ShieldCheck
} from "lucide-react";
...
export default function Profile() {
  ...
  const [formData, setFormData] = useState({
    name: "", age: "", gender: "",
    bloodGroup: "", address: "", phoneNumber: ""
  });
    ...
    <div className="min-h-screen bg-[#0B1120] font-sans text-slate-50 relative overflow-x-hidden w-full">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 py-4 lg:px-12 w-full bg-[#0B1120]/80 backdrop-blur-md border-b border-slate-800/50">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-lg sm:text-xl font-bold text-white">HealthBot</span>
        </Link>
        <div className="flex items-center gap-3 sm:gap-4">
          <Link to="/chat" className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-slate-400 hover:text-teal-400 transition-colors">
            <ArrowLeft className="h-3 w-3 sm:h-4 sm:w-4" /> <span className="hidden sm:inline">Back to Chat</span><span className="sm:hidden">Chat</span>
          </Link>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 sm:py-10 z-10 relative">
        ...
        {/* Message */}
        {message.text && (
          <div className={`mb-6 p-4 rounded-xl text-sm font-medium text-center animate-in fade-in zoom-in duration-300 ${
            message.type === "success"
              ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-400"
              : "bg-rose-500/10 border border-rose-500/30 text-rose-400"
          }`}>
            {message.type === "success" && <CheckCircle2 className="inline h-4 w-4 mr-2 mb-0.5" />}
            {message.text}
          </div>
        )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 sm:gap-4 mb-8">
            <div className="flex items-center gap-4">
              ...
              <div className="overflow-hidden">
                <h1 className="text-xl sm:text-2xl font-bold text-white truncate">{profile?.name}</h1>
            ...
            <div className="w-full sm:w-auto">
              {!editing ? (
                ...
                <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  <button
                    onClick={() => { setEditing(false); setMessage({ text: "", type: "" }); }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl border border-transparent truncate">{profile?.name || "—"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="text-slate-400 text-sm py-3 px-4 bg-slate-800/30 rounded-xl flex items-center justify-between border border-transparent overflow-hidden">
                ...
                {profile?.isVerified && <CheckCircle2 className="h-4 w-4 text-emerald-400 shrink-0" />}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl border border-transparent">{profile?.phoneNumber || "—"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl border border-transparent">{profile?.age || "—"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl capitalize border border-transparent">{profile?.gender || "—"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl border border-transparent">{profile?.bloodGroup || "—"}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 sm:col-span-2">
              ...
              {editing ? (
                ...
                <p className="text-white text-sm py-3 px-4 bg-slate-800/30 rounded-xl border border-transparent">{profile?.address || "—"}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#111827]/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          {/* Header - THE MOBILE FIX: Stack items on mobile so buttons aren't squished */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button
              onClick={() => { setShowPasswordForm(!showPasswordForm); setMessage({ text: "", type: "" }); }}
          ...
          {!showPasswordForm ? (
            <p className="text-slate-400 text-xs sm:text-sm">Your password is securely encrypted. Click "Change Password" to update it.</p>
          ) : (
            <form onSubmit={handlePasswordChange} className="flex flex-col gap-4 animate-in fade-in slide-in-from-top-4 duration-300" autoComplete="off">
              {/* Chrome Autofill Trap */}
              <div className="absolute opacity-0 w-0 h-0 overflow-hidden pointer-events-none -z-50">
                <input type="text" name="prevent_autofill_email" autoComplete="username" tabIndex="-1" />
                <input type="password" name="prevent_autofill_password" autoComplete="current-password" tabIndex="-1" />
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">Current Password</label>
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowCurrentPw(!showCurrentPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">New Password</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                  <input
                    ...
                    className="w-full bg-[#0B1120] border border-slate-700 rounded-xl py-3 pl-12 pr-12 text-sm text-white focus:outline-none focus:border-teal-400 transition-all"
                  />
                  <button type="button" onClick={() => setShowNewPw(!showNewPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white">
                ...
                {passwordData.newPassword.length > 0 && (
                  <div className="flex flex-col gap-1.5 mt-1 ml-1 bg-[#0B1120] p-3 rounded-xl border border-slate-800/50">
                    <p className={`text-[10px] sm:text-xs flex items-center gap-1.5 transition-colors ${passwordData.newPassword.length >= 8 ? "text-emerald-400" : "text-slate-500"}`}>
                      {passwordData.newPassword.length >= 8 ? "✓" : "○"} At least 8 characters
                    </p>
                    <p className={`text-[10px] sm:text-xs flex items-center gap-1.5 transition-colors ${/[A-Z]/.test(passwordData.newPassword) ? "text-emerald-400" : "text-slate-500"}`}>
                      {/[A-Z]/.test(passwordData.newPassword) ? "✓" : "○"} One uppercase letter
                    </p>
                    <p className={`text-[10px] sm:text-xs flex items-center gap-1.5 transition-colors ${/[a-z]/.test(passwordData.newPassword) ? "text-emerald-400" : "text-slate-500"}`}>
                      {/[a-z]/.test(passwordData.newPassword) ? "✓" : "○"} One lowercase letter
                    </p>
                    <p className={`text-[10px] sm:text-xs flex items-center gap-1.5 transition-colors ${/[0-9!@#$%^&*(),.?":{}|<>]/.test(passwordData.newPassword) ? "text-emerald-400" : "text-slate-500"}`}>
                      {/[0-9!@#$%^&*(),.?":{}|<>]/.test(passwordData.newPassword) ? "✓" : "○"} One number or special character
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">Confirm New Password</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                  <input
                    ...
                    className="w-full bg-[#0B1120] border border-slate-700 rounded-xl py-3 pl-12 pr-12 text-sm text-white focus:outline-none focus:border-teal-400 transition-all"
                  />
                  <button type="button" onClick={() => setShowConfirmPw(!showConfirmPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white">
                    {showConfirmPw ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                </div>
                {passwordData.confirmPassword && (
                  <p className={`text-[10px] sm:text-xs mt-1 ${passwordData.newPassword === passwordData.confirmPassword ? "text-emerald-400" : "text-rose-400"}`}>
