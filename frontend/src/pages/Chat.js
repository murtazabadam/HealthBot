...
export function ChatDashboard() {
  ...
  // Prescriptions get their own tab, separate from the chat-attached-image
  // flow (sendPrescriptionImage below) — this lets someone go straight to
  // Prescriptions and upload a photo without typing anything in chat first.
  ...
  const uploadPrescriptionImage = async (base64Image) => {
    if (!token || uploadingPrescription) return;
    ...
    try {
      ...
      if (prescription) setPrescriptions((prev) => [prescription, ...prev]);
      if (createdReminders && createdReminders.length) {
        setReminders((prev) => [...createdReminders, ...prev]);
      }
      showToast(message || "Prescription processed.");
    } catch (err) {
      ...
      showToast(
        err.response?.data?.message ||
          "Couldn't process that prescription image. Try a clearer, well-lit photo.",
      );
    } finally {
  ...
  const handlePrescriptionFileSelect = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => uploadPrescriptionImage(reader.result);
    ...
    try {
      ...
      if (coords) {
        ...
        if (geoData && geoData.length > 0) {
          ...
          throw new Error(
            "Could not find exact map coordinates for your saved address. Try clicking 'Use My Location'.",
          );
        }
      } else {
        throw new Error(
            ...
            <div className="max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500 pb-6">
              ...
              <div className="flex justify-between items-center mb-8 flex-wrap gap-3">
                ...
                <button
                  ...
                  className="bg-teal-500 text-slate-900 px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:brightness-110 transition-all text-sm disabled:opacity-50"
                >
                  {uploadingPrescription ? (
                    ...
                    <>
                      <Paperclip size={16} /> Upload Prescription
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="grid gap-4">
                ...
                {!prescriptionsLoading && prescriptions.length === 0 && (
                  ...
                  >
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      Upload a photo and I'll read the medicines and set up
                ...
                {prescriptions.map((p) => (
                  <div
                    key={p._id}
                    className={`border rounded-2xl p-4 sm:p-6 backdrop-blur-md shadow-sm ${isDark ? "bg-[#111827]/80 border-slate-700/50" : "bg-white border-slate-200"}`}
                  >
                    ...
                    {p.medicines && p.medicines.length > 0 ? (
                      <div className="grid gap-2 sm:grid-cols-2">
                        {p.medicines.map((m, i) => (
                          <div
                            key={i}
                            className={`rounded-xl p-3 border ${isDark ? "bg-slate-900/50 border-slate-800" : "bg-slate-50 border-slate-200"}`}
                          >
                            <p
                      ...
                      <p className="text-sm text-slate-500 italic">
                        No medicines could be confidently read from this
                        image.
                      </p>
                    )}
                    ...
                    {p.rawText && (
                      <details className="mt-4 group">
                        <summary
                          className={`text-xs font-bold uppercase tracking-wide cursor-pointer flex items-center gap-1 ${isDark ? "text-slate-400 hover:text-teal-400" : "text-slate-500 hover:text-teal-600"}`}
                        >
                          <ChevronDown
                            size={14}
                            className="group-open:rotate-180 transition-transform"
                          />
                          View raw scanned text
                        </summary>
                        <p
                          className={`text-xs mt-2 p-3 rounded-xl whitespace-pre-wrap ${isDark ? "bg-slate-900/50 text-slate-400" : "bg-slate-50 text-slate-600"}`}
                        >
                          {p.rawText}
                        </p>
                      </details>
