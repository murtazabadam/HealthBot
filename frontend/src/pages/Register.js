...
export default function Register() {
  ...
  const handleContinue = async (e) => {
    ...
    try {
      ...
      setErrorMessage(
        err.message ||
          "Error sending OTP. Please check your internet connection.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.message || "Registration failed. Invalid OTP.");
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setTimer(60);
      setOtp(["", "", "", "", "", ""]);
      setErrorMessage("A new OTP has been sent to your email."); // Show success message
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#0B1120] font-sans text-slate-50 relative flex flex-col items-center overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col justify-center items-center w-full px-4 z-10 my-10">
        <div className="bg-[#111827]/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-6 sm:p-12 w-full max-w-[850px] shadow-2xl relative">
          ...
          {errorMessage && (
            <div
              className={`mb-6 border text-xs p-4 rounded-xl text-center font-bold animate-pulse ${errorMessage.includes("A new OTP") ? "bg-teal-500/10 border-teal-500/50 text-teal-400" : "bg-red-500/10 border-red-500/50 text-red-400"}`}
            >
              {errorMessage}
            </div>
          )}
            ...
            <>
              <div className="flex flex-col items-center mb-10 text-center">
                ...
                <p className="text-slate-400 text-sm">
                  Join{" "}
                  <span className="text-teal-400 font-bold">HealthBot</span> and
                  start your smarter health journey today.
                </p>
              </div>
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full mt-2 bg-gradient-to-r from-teal-400 to-blue-500 text-slate-900 font-extrabold py-4 rounded-xl shadow-lg uppercase tracking-wide hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 flex justify-center items-center gap-2"
                >
